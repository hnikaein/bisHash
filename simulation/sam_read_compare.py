...
with open(analyze_file, "r") as f:
    ...
    while True:
        ...
        total_aligned_set.add(read_id)
        if len(read_pos) == 0:
            continue
        if read_pos[0][0] != parts[2]:
            continue
        if abs(read_pos[0][1] - int(parts[3])) <= delta:
            if read_id in corrects and is_paired:
                corrects2.add(read_id)
            else:
                corrects.add(read_id)
...
total_aligned = len(total_aligned_set)
correct = len(corrects) + len(corrects2)
